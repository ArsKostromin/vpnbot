...
class BuySubscriptionView(APIView):
    ...
    def post(self, request):
        ...
        # Проверка на активную подписку с той же категорией
        active_subs = user.subscriptions.filter(is_active=True, end_date__gt=timezone.now())
        same_category_sub = active_subs.filter(plan__category=plan.category).first()

        if same_category_sub:
            start_date = same_category_sub.end_date
        else:
